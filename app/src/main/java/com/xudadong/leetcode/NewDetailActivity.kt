...
class NewDetailActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val model = intent?.getSerializableExtra(KEY_MODEL) as Model?

        viewPager2.adapter = object: FragmentStateAdapter(this) {
            override fun getItemCount() = 2

            override fun createFragment(position: Int): Fragment {
                return (if (position == 0) DescriptorFragment.instance(model!!)
                        else CodeViewerFragment.instance(model!!.javaClass.canonicalName!!))!!
            }
        }
    ...
    class DescriptorFragment: Fragment() {
        ...
        private fun inflateDetailView(vDetail: TextView, model: Model) {
            ...
            if (model is RegularModel<*, *>) {
                ...
                vDetail.append(inflateResult(model, regularModel!!.getResult(regularModel.execute(regularModel!!.input))))
            }
        }
        ...
        private fun inflateResult(model: Model, string: String): SpannableString {
            ...
            if (model.timeComplexity != null) {
                sb.append("\n").append("时间复杂度: ").append(model.timeComplexity!!.toString()).append("\n")
                if (model.timeComplexity!!.bestComplexity != null) {
                    sb.append("最优时间复杂度: ").append(model.timeComplexity!!.bestComplexity.toString()).append("\n")
                }
                if (model.timeComplexity!!.worstComplexity != null) {
                    sb.append("最差时间复杂度: ").append(model.timeComplexity!!.worstComplexity.toString()).append("\n")
                }
            }
            if (model.spaceComplexity != null) {
                sb.append("\n").append("空间复杂度: ").append(model.spaceComplexity!!.toString()).append("\n")
                if (model.spaceComplexity!!.bestComplexity != null) {
                    sb.append("最优空间复杂度: ").append(model.spaceComplexity!!.bestComplexity.toString()).append("\n")
                }
                if (model.spaceComplexity!!.worstComplexity != null) {
                    sb.append("最差空间复杂度: ").append(model.spaceComplexity!!.worstComplexity.toString()).append("\n")
                }
            }
        ...
        companion object {
            fun instance(model: Model): DescriptorFragment {
                val fragment = DescriptorFragment()
                val bundle = Bundle()
